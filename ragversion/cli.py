...
@main.command()
def setup():
    """Interactive setup wizard for RAGVersion.

    This wizard helps you configure RAGVersion for your project by asking
    a series of questions about your preferences and requirements.
    """
    ...
    try:
        ...
        console.print(Panel.fit(
            "[bold cyan]RAGVersion Setup Wizard[/bold cyan]\n"
            "[dim]Let's configure RAGVersion for your project[/dim]",
            border_style="cyan",
        ))

        console.print("\n[bold]Step 1: Storage Backend[/bold]")
        console.print("RAGVersion supports two storage backends:\n")
        console.print("  [cyan]sqlite[/cyan]  - Local database (recommended for development)")
        console.print("  [cyan]supabase[/cyan] - Cloud PostgreSQL (recommended for production)\n")
        ...
        config_lines = ["# RAGVersion Configuration\n", "# Generated by setup wizard\n\n"]

        if storage_type == "sqlite":
            console.print("\n[bold]Step 2: SQLite Configuration[/bold]")
            ...
            console.print(f"\n[green]✓[/green] SQLite will use: {db_path}")

        else:  # supabase
            console.print("\n[bold]Step 2: Supabase Configuration[/bold]")
            console.print("\nYou'll need your Supabase project credentials.")
            console.print("Find them at: https://supabase.com/dashboard/project/_/settings/api\n")

            supabase_url = Prompt.ask("Supabase URL")
            supabase_key = Prompt.ask("Service key (or anon key)", password=True)

            # Create .env file
            env_exists = Path(".env").exists()
            if env_exists:
                overwrite = Confirm.ask(".env file exists. Overwrite?", default=False)
                if not overwrite:
        ...
        console.print("\n[bold]Step 3: Tracking Configuration[/bold]")

        store_content = Confirm.ask(
            "Store full content in database?",
            default=True
        )

        max_file_size = Prompt.ask(
        ...
        config_lines.extend([
            "tracking:\n",
            f"  store_content: {str(store_content).lower()}\n",
            f"  max_file_size_mb: {max_file_size}\n",
            "  hash_algorithm: sha256\n\n"
        ])

        # File patterns
        console.print("\n[bold]Step 4: File Patterns (Optional)[/bold]")

        if Confirm.ask("Configure default file patterns?", default=False):
            patterns = Prompt.ask(
                "File patterns (comma-separated, e.g., *.pdf,*.docx)",
                default="*"
            )
        ...
        # Write config file
        config_file = "ragversion.yaml"
        if Path(config_file).exists():
            if not Confirm.ask(f"{config_file} exists. Overwrite?", default=True):
        ...
        if storage_type == "supabase":
            console.print(f"[bold]Environment file:[/bold] .env")

        console.print("\n[bold]Next steps:[/bold]")

        if storage_type == "supabase":
            console.print("  1. Run migrations: [cyan]ragversion migrate[/cyan]")
            console.print("  2. Track your first file: [cyan]ragversion track document.pdf[/cyan]")
        else:
            console.print("  1. Track your first file: [cyan]ragversion track document.pdf[/cyan]")
            console.print("  2. Track a directory: [cyan]ragversion track ./documents[/cyan]")

        console.print("\n[dim]Tip: Use Python API with zero config:[/dim]")
        console.print("[dim]  tracker = await AsyncVersionTracker.create()[/dim]")
...
async def track(
    ...
    try:
        ...
        if p.is_file():
            ...
            def update_progress_callback(event):
                """Callback to collect events for progress display."""
                tracked_events.append(event)

            # Register callback
            tracker.on_change(update_progress_callback)
            ...
            with Progress(
                ...
                # Scan phase
                scan_task = progress.add_task("Scanning and tracking files...", total=None)
